<script lang="ts">
  ...
  let tab = $state<"duration" | "preference">("duration");

  let workCustom = $state("");
  ...
  <div class="content">
    ...
    {#if tab === "duration"}
      ...
      <section>
        ...
        <DurationGrid
          ...
          selected={config.workMinutes}

          unit={t("minutes")}

          customPlaceholder={t("custom")}

          onselect={(m) => update({ workMinutes: m })}

          showCustom

          customValue={workCustom}

          oncustomChange={(v) => (workCustom = v)}

          oncustomConfirm={() => {

            const n = parseInt(workCustom, 10);

            if (n > 0) update({ workMinutes: n });
      ...
      <section>
        ...
        <h3>{t("shortBreakDuration")}</h3>

        <DurationGrid

          presets={BREAK_PRESETS}

          selected={config.shortBreakMinutes}

          unit={t("minutes")}

          customPlaceholder={t("custom")}

          onselect={(m) => update({ shortBreakMinutes: m })}
      ...
      <section>
        ...
        <h3>{t("longBreakDuration")}</h3>

        <DurationGrid

          presets={BREAK_PRESETS}

          selected={config.longBreakMinutes}

          unit={t("minutes")}

          customPlaceholder={t("custom")}

          onselect={(m) => update({ longBreakMinutes: m })}
    ...
    {:else}
      ...
      <SettingCard
        ...
        title={t("longBreakMode")}

        description={t("longBreakModeDesc")}

        checked={config.longBreakMode}

        onchange={(v) => update({ longBreakMode: v })}
      ...
      <SettingCard
        ...
        title={t("strictBreakMode")}

        description={t("strictBreakModeDesc")}

        checked={config.strictBreakMode}

        onchange={(v) => update({ strictBreakMode: v })}
      ...
      <SettingCard
        ...
        title={t("autoBreakMode")}

        description={t("autoBreakModeDesc")}

        checked={config.autoBreakMode}

        onchange={(v) => update({ autoBreakMode: v })}
      ...
      <SettingCard
        ...
        checked={config.dailyFocusGoal}

        onchange={(v) => update({ dailyFocusGoal: v })}
      ...
      >
        ...
        {#if config.customThemeColor}
          ...
          <ColorPicker
            ...
            onchange={(c) => update({ themeColor: c })}
